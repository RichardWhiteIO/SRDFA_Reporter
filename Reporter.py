...
try:
    symmetrix_list = requests.get(vURL, auth=(
        USER, PASS), verify=False).json()["symmetrixId"]

except ValueError:
    logger.critical(
        'Login failure. $SRUSER and $SRPASS credentials failed. Please confirm the environmental variables are properly set with correct username/password')
    sys.exit(1)
except Exception as e:
    logger.critical(
        'IP:Port information is incorrect. Please confirm SRUNILOCATION environment variable is properly set and Unisphere is up and running. Detailed Error: {}'.format(e))
    sys.exit(1)
...
for symmetrix_id in symmetrix_list:
    ...
    if 'message' in sresponse:
        message = sresponse.get('message') + ". " + \
            symmetrix_id + " array is being skipped."
        logger.warn(message)
        continue
    else:
    ...
    for storage_group_id in storage_group_list:
        ...
        ssr = requests.post(ss_url, data=json.dumps(data, sort_keys=True, indent=4), auth=(USER, PASS), headers=headers, verify=False)
        ssresponse = ssr.json()
        ...
        if 'message' in ssresponse:
            message = ssresponse.get('message') + ". " + \
                symmetrix_id + " array is being skipped."
            logger.warn(message)
            break
        ...
        print("Writing to file: {0}, {1}, {2}".format(
            timestamp, symmetrix_id, storage_group_id))
